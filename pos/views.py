...
@login_required
@vendor_required
def pos_event_detail(request, agent_id, event_id):
    # Get the event by ID
    event = get_object_or_404(Event, id=event_id)

    # Get the POS agent by ID (the agent selected in the previous view)
    pos_agent = get_object_or_404(User, id=agent_id, is_posagent=True)
    
    # Fetch the associated agent assignment for the current vendor and selected agent
    agent_assignment = get_object_or_404(AgentEventAssignment, event=event, vendor=request.user, agent=pos_agent)

    # Fetch tickets generated by the selected agent for the event
    generated_tickets = Ticket.objects.filter(event=event, generated_by=agent_assignment.agent)
    
    # Fetch tickets verified by the selected agent for the event
    ...
    # Check if verifying tickets is allowed for the selected agent
    verifying_enabled = agent_assignment.verifying_tickets if agent_assignment else False
    verified_totals = []
    ...
    context = {
        ...
        'pos_agent': pos_agent,  # Pass the selected agent to the context
    }
