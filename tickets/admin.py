...
class TicketAdmin(admin.ModelAdmin):
    list_display = ('ticket_number', 'event', 'user', 'customer_username', 'ticket_category', 'purchase_date', 'verified', 'entity_type','verified_by')
    list_filter = ('verified', 'entity_type', 'event', 'user','verified_by')
    search_fields = ('ticket_number', 'customer_username', 'user__username', 'event__title')
    ordering = ('-purchase_date',)
    readonly_fields = ('qr_code', 'purchase_date','verified_by')  # QR code and purchase date should be read-only
